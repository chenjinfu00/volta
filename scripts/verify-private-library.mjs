...
const source=JSON.parse(await fs.readFile(path.join(libraryData(libraryRoot()),'catalog.json')));
...
if(missingChunks.length){
  await fs.writeFile(path.join(libraryData(libraryRoot()),'cloud-missing-chunks.json'),JSON.stringify(missingChunks,null,2));
...
await fs.writeFile(path.join(libraryData(libraryRoot()),'cloud-verification.json'),JSON.stringify(report,null,2));
