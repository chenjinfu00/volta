...
const root=path.resolve(import.meta.dirname,'..'),out=path.join(root,'dist/volta');
await fs.mkdir(out,{recursive:true});
await fs.cp(path.join(root,'docs'),out,{recursive:true,filter:source=>!['scores','fit','library'].includes(path.relative(path.join(root,'docs'),source).split(path.sep)[0])});
