...
批注保存在浏览器里。选了曲谱文件夹之后，还可以让它跟着曲谱走：

- **Mac 上的 Chrome / Edge**：批注自动写进 `曲谱库数据/批注/<id>.json`，一首一份。放在 iCloud、Dropbox 里就自动同步了。
- **iPad / Safari**：Safari 不支持网页自动写入《文件》App 里的指定文件夹（加到主屏幕也一样）。iPad 上的批注会在书写时自动保存在 Volta 的本机数据库，平时不需要点保存或选择位置；文件夹里已有的批注仍会自动读进来。需要把备份带回曲谱文件夹时，再点设置里的「导出这份曲谱的批注」，通过分享面板存回去。
- 合并时两边的笔迹都保留。同一笔出现两个不同版本时会停下来报错，不会替你挑一个。

---

## 离线

`选择文件夹` 之后就不需要网络了。应用本体由 Service Worker 缓存，目录、PDF、MIDI、页面边界全部从文件夹读。

设置里的 **一键部署到本机** 会把应用和你勾选的曲谱一起存进浏览器（存之前核对 SHA-256）。存过的曲谱之后连文件夹都不用再选——对 iPad 特别有用，因为 Safari 每次重开都要重新选一次文件夹。
