...
Safari/iPad 的网页文件夹选择通常只能读取《文件》App 中的文件，不能申请写入权限；批注工具栏的“导出”会生成一个 `volta-annotations.json`，在系统分享面板中选择“存储到文件”，再放进 `曲谱库数据/批注/` 即可；如果直接放在 `曲谱库数据/` 根目录，应用也能识别。原生 Volta 应用可以使用系统文件权限长期写回曲谱库。
