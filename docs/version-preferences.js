...
export function versionPreferences(fetcher=fetch,{localOnly=PUBLIC_LIBRARY,storage=globalThis.localStorage}={}){
  return {
    async load(workKey){
      if(localOnly){try{return storage.getItem('volta:version:'+workKey)||null;}catch{return null;}}
    ...
    async save(workKey,sourceId){
      if(localOnly){storage.setItem('volta:version:'+workKey,sourceId);return;}
