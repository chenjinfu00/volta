...
export function readAll(storage=localStorage){
...
export function readBookmarks(scoreId,storage=localStorage){return sortBookmarks(readAll(storage)[scoreId]);}
export function writeBookmarks(scoreId,list,storage=localStorage){
  if(!scoreId)return;
...
export function setupBookmarks({score=()=>null,page=()=>1,jump=()=>{},canJump=()=>true,storage=localStorage,toast=()=>{}}={}){
  ...
  function render(){
    ...
    for(const item of items){
      ...
      remove.onclick=()=>{items=removeBookmark(items,item.page);writeBookmarks(id,items,storage);render();};
  ...
  add.onclick=()=>{
    ...
    writeBookmarks(id,items,storage);label.value='';render();
  };
  label.onkeydown=event=>{if(event.key==='Enter'){event.preventDefault();add.click();}};
  render();
  return {
    setScore(score){id=score?.id||null;items=id?readBookmarks(id,storage):[];render();},
    refresh:render,
