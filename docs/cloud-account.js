...
export async function requireCloudLogin(){
  if(CLOUD_HOME&&location.hostname.endsWith('.github.io')){location.replace(CLOUD_HOME);return false;}
  if(!CLOUD_LIBRARY)return true;
  const dialog=$('cloud-login'),status=$('cloud-login-status');
  try{
    const response=await cloudRequest('session');if(!response.ok)throw Error('服务暂不可用');
    const data=await response.json();localStorage.setItem(trustedKey,JSON.stringify(data.device));return true;
  }catch(error){
    let trusted;try{trusted=JSON.parse(localStorage.getItem(trustedKey));}catch{}
    if(trusted&&(!navigator.onLine||['TypeError','TimeoutError'].includes(error.name))){$('cloud-status').textContent='离线模式 · 仅使用这台设备已下载的曲谱与批注';return true;}
  ...
  return new Promise(resolve=>{
    $('cloud-login-form').onsubmit=async event=>{
      ...
      try{
        ...
        localStorage.setItem(trustedKey,JSON.stringify(data.device));$('cloud-password').value='';dialog.close();resolve(true);
