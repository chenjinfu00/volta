...
test('offline shell includes all local modules, PDF engine resources and no scores or remote font requests',async()=>{
  ...
  for(const path of ['./index.html','./reader.css','./app.js','./reader-shell.js','./offline.js','./vendor/pdf.worker.mjs','./library/catalog.json'])assert.ok(manifest.includes(path),path);
