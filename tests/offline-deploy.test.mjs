...
test('the service worker saves the shell in batches and answers the page',async()=>{
  ...
  assert.match(sw,/volta-shell-20260919-1255-c/,'a published shell update gets a fresh cache');
  ...
  const deploy=await fs.readFile(new URL('../docs/offline-deploy.js',import.meta.url),'utf8');
  assert.match(offline,/sw\.js\?version=20260919-1255/,'normal startup asks the browser to check the new worker');
  assert.match(deploy,/sw\.js\?version=20260919-1255/,'manual deployment asks for the same worker version');
  const html=await fs.readFile(new URL('../docs/index.html',import.meta.url),'utf8');
