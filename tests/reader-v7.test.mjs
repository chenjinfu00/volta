...
test('local collection stays out of public assets and has a complete distinct classified PDF set',async t=>{
  let root;try{root=libraryRoot();}catch{t.skip('Private collection is not available at the configured library path');return;}
  const data=libraryData(root),catalog=JSON.parse(await fs.readFile(path.join(data,'catalog.json')));
  const manifest=JSON.parse(await fs.readFile(path.join(data,'manifest.json')));
...
test('local server serves only registered PDFs and metadata, with byte ranges and no private index access',async t=>{
  let library;try{library=libraryRoot();}catch{t.skip('Private collection is not available at the configured library path');return;}
