...
test('physical folders are the Volta shelf, level for level',()=>{
  ...
  assert.equal(animenz.relative,'动漫/红莲の弓矢/红莲の弓矢 · bbbbbbbb.pdf');
  const animenzSuite=libraryRelativePath(pdf('m','Animenz（编曲） - key社组曲串烧',{composer:'Animenz',arranger:'Animenz',style:'动漫／影视'}),{shelfWorks:100});
  assert.equal(animenzSuite.relative,'动漫/key社组曲串烧/key社组曲串烧 · mmmmmmmm.pdf');
  const honour=libraryRelativePath(pdf('c','王者荣耀 - 曲谱合集',{composer:'王者荣耀',style:'游戏音乐'}));
  assert.equal(honour.relative,'流行音乐与其他游戏/曲谱合集/曲谱合集 · cccccccc.pdf');
  ...
  assert.equal(pop.relative,'流行音乐与其他游戏/给你们/给你们 · ffffffff.pdf','a generic genre does not add another folder');
  const wuthering=libraryRelativePath(pdf('k','鸣潮 - 愿戴荣光坠入天渊',{composer:'鸣潮',style:'游戏音乐'}));
  assert.equal(wuthering.relative,'流行音乐与其他游戏/愿戴荣光坠入天渊/愿戴荣光坠入天渊 · kkkkkkkk.pdf');
  const other=libraryRelativePath(pdf('l','帕赫贝尔 - D大调卡农',{composer:'帕赫贝尔',style:'巴洛克',category:'其他'}),{shelfWorks:25});
  assert.equal(other.relative,'其他/D大调卡农/D大调卡农 · llllllll.pdf');
  const chopin=libraryRelativePath(pdf('n','肖邦 - 船歌, Op.60',{composer:'弗雷德里克·肖邦',style:'浪漫主义'}),{shelfWorks:150});
  assert.equal(chopin.relative,'弗雷德里克·肖邦/抒情小品/船歌, Op.60/船歌, Op.60 · nnnnnnnn.pdf');
  const beethoven=libraryRelativePath(pdf('o','贝多芬 - 第九交响曲',{composer:'路德维希·范·贝多芬',style:'古典主义'}),{shelfWorks:10});
  assert.equal(beethoven.relative,'路德维希·范·贝多芬/第九交响曲/第九交响曲 · oooooooo.pdf');
  const chen=libraryRelativePath(pdf('p','陈致逸 - 嬉皮之舞',{composer:'陈致逸',style:'当代钢琴'}),{shelfWorks:8});
  assert.equal(chen.relative,'陈致逸/嬉皮之舞/嬉皮之舞 · pppppppp.pdf');
  const curated=libraryRelativePath(pdf('g','陈致逸 - 嬉皮之舞',{composer:'陈致逸',style:'当代钢琴'}),{current:'陈致逸/练习曲/旧文件.pdf'});
  assert.match(curated.relative,/^陈致逸\/嬉皮之舞\//,'a one-genre shelf does not keep a redundant middle layer');
